...
The USGS 3DEP **Seamless 1-meter (S1M)** DEM is a 1-meter elevation dataset covering the parts of the CONUS where high-resolution (mostly lidar-derived) source data exists — coverage is **partial and still growing, not nationwide**. It is distributed as COG + metadata pairs in the public USGS bucket (`s3://prd-tnm/StagedProducts/Elevation/S1M/`, NAD83(2011) Conus Albers / NAVD88). The viewer renders it as a **3D mesh, not as flat imagery**:
...
S1M coverage is partial and still expanding — much of the CONUS has no S1M tile yet — and the viewer renders no terrain mesh where a tile is absent.
