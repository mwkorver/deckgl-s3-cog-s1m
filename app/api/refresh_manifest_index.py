...
Usage:
  ...
  python refresh_manifest_index.py --years-from 2022 \
      --index ../cache/manifest_index s3://cog-stac-catalog/manifest-index
...
def _merge_partition(src_index, dst_index, state, year):
    ...
    if str(dst_index).startswith("s3://"):
        ...
        subprocess.run(["aws", "s3", "rm", "--recursive", dst], check=False,
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        subprocess.run(["aws", "s3", "cp", "--recursive", f"{src}/", dst], check=True,
...
def main():
    ...
    ap.add_argument("--index", nargs="+", required=False, default=[],
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        subprocess.run(
            [sys.executable, str(HERE / "build_manifest_index.py"),
             "--manifest", str(keyfile), "--out", idx_tmp],
            check=True,
        )
