...
| `deckgl-s3-cog-s1m-data.json` | inline policy | `deckgl-s3-cog-s1m-deploy` role | S3 data plane for local CLI / docker-compose ingest (read `naip-analytic`, RW the lake). Also still grants `naip-geoparquet-index`, which nothing functional reads any more — see below. |
...
update it copies whatever the `SeedKeys` parameter lists — by default `lake/`
and `collections.geojson` — from `deckgl-s3-cog-s1m-seed-us-west2` into the
deployer's bucket. (The seed bucket policy also permits `anchors.geojson`, but
no such object exists at its root and `SeedKeys` does not request it.) On delete
it leaves the copied objects in place because the bucket is retained.
Cross-account deployments require the seed bucket policy to allow the target
account's seed-copy role to `ListBucket` and `GetObject`.

> **Redeploying foundation re-runs the copy.** It is a create/update handler, so
> an update overwrites objects under `lake/` with the seed versions — silently
> reverting partitions you ingested yourself, and restoring ones you deleted.
> Pass an empty value to suppress it:
>
> ```bash
> aws cloudformation deploy --stack-name deckgl-s3-cog-s1m-foundation \
>   --template-file foundation.yaml --capabilities CAPABILITY_NAMED_IAM \
>   --region us-west-2 --parameter-overrides 'SeedKeys='
> ```
>
> `deploy-foundation.sh` does not pass this, so use the raw command above when
> updating a stack whose lake has diverged from the seed.
...
point the repo at it by setting `AWS_PROFILE` in `app/.env` (the example ships a
`CHANGEME-` placeholder, so this is a required edit, not an override):
...
`role/deckgl-s3-cog-s1m-read-*`, `role/deckgl-s3-cog-s1m-ingest-*`, and the
fixed-name `role/deckgl-s3-cog-s1m-seed-copy` the foundation stack needs.

> **Public collections (KyFromAbove, New Jersey, Indiana)** are read with an
> *unsigned/anonymous* S3 client, which bypasses the role's identity policy — so
> neither role needs (or should get) permissions for `kyfromabove`,
> `njogis-imagery` or `gisimageryingov`.
...
- **Region lock:** the foundation, read, and ingest templates force deployment
  ...
  `njogis-imagery`, `kyfromabove`, and `gisimageryingov`) are hosted there. Keeping compute,
...
- **Unused grant:** `deckgl-s3-cog-s1m-data.json` still allows read on
  `naip-geoparquet-index`. NAIP ingest discovery lists `naip-analytic` directly
  and the viewer reads the lake, so no functional path uses that bucket; only the
  retired `ManifestIndexAdapter` does, and no collection is wired to it. Safe to
  drop from the policy if you are not planning to re-adopt index-backed discovery.
