...
| `deckgl-s3-cog-s1m-data.json` | inline policy | `deckgl-s3-cog-s1m-deploy` role | S3 data plane for local CLI / docker-compose ingest (read `naip-analytic` + `cog-stac-catalog`, RW the lake). |
