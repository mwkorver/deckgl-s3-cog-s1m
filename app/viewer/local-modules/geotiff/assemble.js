 ...
 */
export function assembleTiles(tiles, opts) {
    ...
    const topLeftTile = tiles.find((t) => t.x === minCol && t.y === minRow) ?? tiles[0];
 ...
 * {@link RasterArray}.
 */
function assemblePixelInterleaved(tiles, opts, meta) {
    const { width, height, tileWidth, tileHeight, minCol, minRow } = opts;
    ...
    const Ctor = firstArray.data.constructor;
    const outputData = new Ctor(width * height * count);
    for (const tile of tiles) {
        const { array } = tile;
        if (array.layout !== "pixel-interleaved") {
            throw new Error("All tiles must have the same layout; expected pixel-interleaved");
        }
        const colOffset = (tile.x - minCol) * tileWidth;
        const rowOffset = (tile.y - minRow) * tileHeight;
 ...
 * {@link RasterArray}.
 */
function assembleBandSeparate(tiles, opts, meta) {
    const { width, height, tileWidth, tileHeight, minCol, minRow } = opts;
    const { count, transform, crs, nodata, mask } = meta;
    const firstArray = tiles[0].array;
    ...
    const Ctor = firstArray.bands[0].constructor;
    const outputBands = [];
 ...
 */
function copyRows(src, dst, opts) {
    const { srcWidth, dstWidth, colOffset, rowOffset, copyWidth, copyHeight, samplesPerPixel, } = opts;
    ...
    for (let row = 0; row < copyHeight; row++) {
        ...
        const dstStart = (rowOffset + row) * dstStride + colOffset * samplesPerPixel;
 ...
 * the expected range.
 */
function validateContiguousGrid(tiles, opts) {
    if (tiles.length === 0) {
    ...
    if (tiles.length !== expectedCount) {
        throw new Error(`Expected ${expectedCount} tiles for a ${numCols}x${numRows} grid, got ${tiles.length}`);
    }
    const seen = new Set();
    for (const tile of tiles) {
        ...
        if (col < 0 || col >= numCols || row < 0 || row >= numRows) {
            throw new Error(`Tile (${tile.x}, ${tile.y}) is outside the expected grid range ` +
