...
* A debounced listener waits for `300ms` of view stability and calls [`refreshS1MTerrain()`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L3869).

### 2. Fetching the Active S1M Grid
* [`refreshS1MTerrain()`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L3869) issues a request to the backend `/s1m/tiles` endpoint, passing the padded viewport bounding box.
* The API returns the list of active 3D terrain grid tiles (DEM files).
* The client sorts the returned tiles from the bottom of the viewport upward ([`sortS1MTilesBottomFirst()`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L3902)) so foreground terrain is processed and starts draping first.

### 3. Subdivision (Sub-tiling) Decision
* A single S1M grid tile spans a massive ~12 km footprint. To avoid stretching a single low-resolution texture across this area, the client splits each tile into an $N \times N$ grid of smaller sub-tiles ([`s1mSubdiv()`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L3259)).
* For collections with smaller-sized COG tiles like **New Jersey** (`nj-imagery`) or **Kentucky** (`kyfromabove`), [`isSmallCogCollection()`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L1862) returns `true`. This scales up subdivision aggressively, using a minimum subdivision level of $2 \times 2$ (~6 km sub-tiles) and up to $6 \times 6$ sub-tiles when zoomed in.

### 4. Querying Intersecting COGs (STAC Search)
* For each visible sub-tile, [`ensureSubTileDrape()`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L3382) queries the backend `/search` endpoint to find all imagery files that spatially cover the sub-tile:
  ```json
  {
...
* The client passes the S3 path to [`resolveGeotiffSource()`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L1647). If asset signing is active, it requests a signed URL. For public buckets, it maps the path directly to the region-specific virtual-hosted S3 URL:
...
* The client compares the span of the sub-tile bounding box with the span of the COG tile via [`chooseDrapeImageLevel()`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L3053).
...
* The client maps the sub-tile's corners to the COG overview coordinates using [`drapePixelMapper()`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L3017), which applies the inverse georeferencing matrix (`affine.invert(level.transform)`).
* The client requests the necessary internal tiles (e.g., $512 \times 512$ pixel blocks) using HTTP `Range` requests (`fetchTile`).
* Decoded tiles are stored in a Shared LRU Cache ([`s1mCogTileCache`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L2968)) so overlapping boundaries between sub-tiles share chunks instead of duplicating network requests.
...
* [`displayDrapeRgbaBytes()`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L2911) uses [`collectionForHref()`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L908) to fetch the display range (`display: { domain: [domainMin, domainMax] }`) from the collection registry.
...
* The client initializes a blank $384 \times 384$ canvas buffer via [`buildDrapeImage()`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L3229).
* For each coordinate $(u,v)$ in the canvas, [`paintDrapeSource()`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L3079):
...
* [`buildS1MSubTileLayerGPU()`](file:///Users/mwkorver/working/deck-s3-naip-s1m-obf/app/viewer/index.html#L3454) instantiates a `TerrainMeshLayerClass` for deck.gl, linking:
