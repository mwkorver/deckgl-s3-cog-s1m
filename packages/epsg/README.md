...
Currently, the only package entrypoint is `@s3-cog/epsg/all`, which loads a `Map<number, string>`, with all EPSG definitions in OGC WKT2 format.

```ts
import loadEPSG from "@s3-cog/epsg/all";
