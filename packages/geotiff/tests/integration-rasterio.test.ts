...
/**
 * The .npy reference tiles are generated, not committed: `geotiff-test-data`
 * ships the source .tif files but you must run `pixi run generate-npy` inside
 * fixtures/geotiff-test-data to produce the arrays these tests compare against.
 * Skip (rather than fail) when they're absent so a plain `pnpm test` on a fresh
 * clone is green, and the skip is visible in the test output instead of being
 * hidden behind a CI --filter exclusion.
 */
const NPY_FIXTURES_PRESENT = existsSync(
  fixturePath(FIXTURES[0].name, FIXTURES[0].variant, "/0-0-0.npy"),
);

if (!NPY_FIXTURES_PRESENT) {
  console.warn(
    "[integration-rasterio] skipping: no .npy reference tiles found. " +
      "Generate them with `cd fixtures/geotiff-test-data && pixi run generate-npy`.",
  );
}
...
describe.skipIf(!NPY_FIXTURES_PRESENT)("tile data matches", () => {
...
describe.skipIf(!NPY_FIXTURES_PRESENT)("Single-bit mask data", () => {
