...
Integrates with [`@s3-cog/affine`] for easy handling of image transforms, associating pixel positions to spatial coordinates.
...
[`@s3-cog/affine`]: https://developmentseed.org/deck.gl-raster/api/affine/
...
If your object store requires request headers on every range read, pass them to
`GeoTIFF.fromUrl`:

```ts
import { GeoTIFF } from '@s3-cog/geotiff';

const geotiff = await GeoTIFF.fromUrl('https://example.com/cog.tif', {
  headers: {
